...
function handleMouseDown(
  ...
  const nextCell = (
    view.domAtPos(pluginState.activeHandle + 1)!.node as HTMLElement
  ).nextElementSibling as HTMLElement | null;
  const nextCellStartWidth = nextCell?.offsetWidth;
  view.dispatch(
    view.state.tr.setMeta(columnResizingPluginKey, {
      setDragging: {
        startX: event.clientX,
        startWidth: width,
        nextCellStartWidth: nextCellStartWidth,
      },
    })
  );
  ...
  function finish(event: MouseEvent) {
    ...
    if (pluginState?.dragging) {
      ...
      updateColumnWidth(view, pluginState.activeHandle, width, draggedNextCell);
      view.dispatch(
  ...
  displayColumnWidth(
    ...
    width,
    defaultCellMinWidth,
    nextCellStartWidth ? { width: nextCellStartWidth } : undefined
  );
...
function draggedWidth(
  ...
  draggedNextCell?: { width: number | null }
): number {
...
function updateColumnWidth(
  ...
  cell: number,
  width: number,
  nextCellWidth?: { width: number | null }
): void {
  const $cell = view.state.doc.resolve(cell);
  const table = $cell.node(-1),
  ...
  if (nextCellWidth?.width && col + 1 < map.width) {
    for (let row = 0; row < map.height; row++) {
      const mapIndex = row * map.width + col + 1;
      // Rowspanning cell that has already been handled
      if (row && map.map[mapIndex] == map.map[mapIndex - map.width]) continue;
      const pos = map.map[mapIndex];
      const attrs = table.nodeAt(pos)!.attrs as CellAttrs;
      const index = attrs['colspan'] == 1 ? 0 : col - map.colCount(pos);
      if (attrs['colwidth'] && attrs['colwidth'][index] == nextCellWidth.width)
        continue;
      const colwidth = attrs['colwidth']
        ? attrs['colwidth'].slice()
        : zeroes(attrs['colspan']);
      colwidth[index] = nextCellWidth.width;
      tr.setNodeMarkup(start + pos, null, { ...attrs, colwidth: colwidth });
    }
  }
